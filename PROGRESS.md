...
## Done
- **판화 질감 브러시(리소 거친/젖은) + 그 사이 버그 수정 묶음** (2026-09-10~17, Claude,
  v2.20.0~v2.22.0): 순서대로.
  - **undo를 캔버스 전체 스냅샷 → 부분영역(dirty-rect)으로 개편**: 스트로크 하나 시작할 때마다
    캔버스 전체(A3 200dpi 최대 ~44MB)를 두 번씩 복사하던 게 "선 많이 그으면 렉" 원인 — 각
    브러시가 실제로 칠한 반경만 `markDirty()`로 추적해 그 부분만 undo에 저장하도록 전면 개편
    (`BrushView.kt`의 `UndoEntry`/`commitStrokeUndo`/`markDirty`).
  - **일기 상세/달력 다운로드에서 종이 Multiply가 안 먹히던 버그**: 예전 백업 동기화 버그(투명
    PNG가 JPEG로 재인코딩되며 검게 깨짐)를 화면에서만 몰래 복구해주는 안전장치
    (`DiaryRepository.buildLegacyDiaryPreviewPixels`)가 옛날 방식(SRC_OVER)으로 재합성해서,
    이 경로를 타는 일기만 저장 시점의 Multiply가 무시됐다 — 복구 로직도 Multiply로 통일.
  - **S펜으로 색상휠 고를 때 호버 상태에서 오작동**: 원형 휠·밝기 막대의 raw pointerInput 루프가
    `pressed` 체크 없이 모든 이벤트(호버 포함)를 처리해서, 펜이 화면에 닿기 전부터 색이 바뀌던
    버그 수정.
  - **일기 달력 오버레이에 "연도 표시" 토글 추가**: `OverlayPlacement.showYear`.
  - **페인트통과 선 사이에 안 채워진 실핏줄**: 정확히 일치하는 색만 채우는 flood fill이 선의
    안티에일리어싱 경계 픽셀을 못 채우던 것 — 채워진 영역과 맞닿은 반투명 경계에 채우기색을
    한 겹 더 깔아주는 `bleedFillUnderEdges()` 추가.
  - **표지 수정 화면의 삭제 버튼 위치**: 자주 누르는 즐겨찾기 별 바로 옆에 파괴적 동작(삭제)이
    붙어있던 걸 반대쪽(닫기 옆)으로 이동.
  - **판화 질감 브러시 2종 신규**(브레인스토밍→스펙→계획→SDD 4태스크, 서브에이전트 병렬 진행
    상황을 보여주는 픽셀아트 대시보드 아티팩트 동원): 기존 연필/크레용처럼 매번 무작위 입자를
    뿌리는 방식이 아니라, 256×256 고정 노이즈 질감 타일을 `BitmapShader(REPEAT)`로 캔버스
    절대좌표에 타일링해서 찍는 새 방식 — 같은 자리를 두 번 칠하면 항상 같은 무늬가 겹쳐 보임
    (리소그래피 판 느낌). Rough=블러 없는 원본 노이즈, Wet=블러+대비 재정규화. 강도(원본
    "70/30·50/70")는 별도 슬라이더 없이 기존 불투명도로 대체. 순수 함수
    `LithoTexture.kt#generateGrainTexture`는 유닛테스트로 검증, 나머지는 컴파일+수동 확인.
    최종 전체 브랜치 리뷰에서 "스탬프 간격이 좁아 같은 자리에 ~10번 겹쳐 찍히면서 불투명도
    100%와 60%가 거의 안 구분될 수 있다"는 Important 발견이 나왔으나, 계획이 명시한 간격 값과
    충돌하는 사안이라 사용자 판단으로 일단 그대로 배포하고 실기에서 확인 후 필요하면 조정하기로
    함(스펙: `docs/superpowers/specs/2026-09-17-litho-texture-brushes-design.md`, 계획:
    `docs/superpowers/plans/2026-09-17-litho-texture-brushes.md`).
- **코드 구조 정리 및 UI·UX 검토** (2026-09-14): HEAD `103c545`와 기존 미커밋 화면 변경을
  기준으로 현재 Kotlin 구조·시안·입력/저장/동기화 경로를 검토했다. README의 구형 rooms/Storage
  설명을 실제 개인/일기/참가자별 공유 구조로 교체하고 plan.md를 역사 자료로 표시했다.
  상세 근거·우선순위는 `docs/reviews/2026-09-14-code-ui-ux-review.md`.
  감성 방향(종이·책·문구·오리)은 일관적이지만 Undo/Redo 저장·공유 알림 누락, 30dp 커스텀 버튼과
  클릭 semantics 부재, 상호 모순된 빈 화면 안내, 다크 로고·표지 제목 저대비를 우선 개선 대상으로
  확인했다. 앱 소스·사용자 수정 PPTX는 변경하지 않았다. ADB 연결 기기가 없었고 단위 테스트·lint는
  `:app:processDebugResources`의 R.jar 삭제 IOException으로 중단되어 통과 여부 미확인.
- **브러시 개선 6건 + 이미지 스포이드 기능** (2026-09-08~09, Claude): v2.19.0 배포(펜 불투명도
  체감 보정, S펜 지우개 버튼 타이밍 수정, 수채화 Multiply 블렌드, 색상휠 밝기 슬라이더 미리보기)
  이후 이어서 작업.
  - **펜 불투명도 감마 보정**: 흰 종이 위 검정 잉크 50%는 산술적으로 정확히 RGB 128이지만 사람
    눈엔 그보다 훨씬 안 옅어 보인다는 리포트 — `BrushView.kt`에 `inkAlpha() = opacity^1.6` 보정을
    추가해 펜/워터의 `composite()` 알파와 펜슬/크레용의 점당 알파에 적용(지우개는 미적용). 표시되는
    슬라이더 숫자는 그대로 선형.
  - **S펜 버튼을 누른 채로 새 스트로크를 시작하면 안 지워지던 버그**: `onStylusButtonChanged` 콜백
    → Compose 상태 → 재구성 → `BrushView.erasing` 반영까지 최소 한 프레임 걸려서, 버튼을 먼저
    누르고 터치를 시작하면 그 첫 점은 그냥 잉크로 그려졌다. `strokeStart`/`strokeMove`가
    `erasing || stylusButtonDown`(같은 터치 이벤트 안에서 동기적으로 감지한 값)을 보도록 수정.
  - **수채화가 밑그림을 덮어 가리던 문제 → Multiply 블렌드**: `composite()`에서 WATER 브러시만
    `BlendMode.MULTIPLY`(API 29+, minSdk 24라 가드)로 합성 — 겹쳐 찍는 반투명 도트가 밑그림을
    가리지 않고 어둡게 물들인다. 이어서 "저장/공유 시 종이 질감도 비쳐 보이면 좋겠다"는 요청으로
    `exportBitmap()`(일기 저장·공유 그리기 동기화·다운로드가 쓰는 "종이+필기 합성" 함수)도 종이와
    Multiply로 합치도록 확장 — 실시간 편집 화면은 그대로 일반 블렌드 유지, 필기 전체(펜 포함)를
    간단하게 한 번에 곱하는 방식으로 스코프를 좁힘(붓별 구분은 안 함, 사용자가 명시적으로 단순한
    쪽 선택).
  - **색상휠 밝기 슬라이더가 항상 흑백만 보여주던 것 수정**: RGB/HSL 슬라이더처럼 지금 고른
    색상·채도를 반영한 그라디언트로 바꿔서 "이 자리로 옮기면 이 색"이 보이게 함.
  - **이미지 스포이드 기능** (브레인스토밍→스펙→계획→SDD 4태스크): 색상 라이브러리 칸 편집 중
    "이미지에서" 버튼으로 갤러리 사진을 열어, 핀치줌·드래그로 확대/이동하며 손가락으로 누르면
    루페 미리보기(기존 `EyedropFloatingPreview` 재사용)가 뜨고 떼면 그 색이 강조된 칸에 즉시
    저장, 스와치 줄에서 대상 칸을 바꿔가며 같은 사진에서 연달아 뽑을 수 있다. 새 파일
    `ImageEyedropper.kt`(화면→비트맵 픽셀 역변환 순수 함수, 유닛 테스트 8개 + 전체화면
    `Dialog` 오버레이, 한 손가락=색뽑기/두 손가락=핀치줌 제스처 판별)와 `ColorPickerCard`에 진입점
    추가. 기존 `decodeCoverBitmap`/`onEditLibraryColor` 경로를 그대로 재사용해 라이브러리
    저장·백업 동기화는 새로 안 만듦. 태스크 리뷰에서 2핀치→1손가락 전환 시 의도치 않은 색이
    커밋되는 버그를 찾아 `hadMultiTouch` 가드로 수정. 최종 전체 브랜치 리뷰에서 확대 시 이미지가
    스와치 줄 밖으로 넘치는 문제, 색상피커 슬라이더 드래그마다 갤러리 런처가 매 프레임(전체 5곳
    호출부에서) 재등록되던 문제, 이미지 디코드 실패 시 아무 반응 없던 문제(스펙에 있던 Toast를
    계획 작성 때 누락), 확대 후 축소하면 이미지가 화면 밖으로 밀려 못 돌아오던 문제를 찾아 한
    수정 웨이브로 전부 해결·재검토 확인(기존 `CoverImageCropContent`/`cropSelectedRegion`
    패턴을 그대로 재사용). 스펙:
    `docs/superpowers/specs/2026-09-09-image-eyedropper-swatch-design.md`, 계획:
    `docs/superpowers/plans/2026-09-09-image-eyedropper-swatch.md`.
  - 전부 master에 직접 커밋(워크트리 없이 진행 — 이 프로젝트 확립된 패턴). 아직 GitHub에 푸시하지
    않음 — 다음 세션에서 사용자 확인 후 배포할 것.
- **색상 라이브러리 기능 구현 및 origin/master 푸시 완료** (2026-09-08, Claude): 21색 고정
  팔레트를 7색씩 묶은 "라이브러리"(사용자가 이름 지음, 최대 30개) 중 최대 3개를 선택해 이어붙인
  파생 팔레트로 재구성했다. 즉시선택 즐겨찾기는 5색→3색으로 줄였다. `ColorLibrary.kt`(순수
  Kotlin, 팔레트 파생·FIFO 활성 선택·CRUD·직렬화, 유닛 테스트 18개)를 새로 추가하고,
  `SessionStore`의 `paletteColors`를 저장값에서 `libraries`/`activeLibraryIds` 기반 계산값으로
  바꿨다. `BrushControls.kt`의 팔레트 팝업을 라이브러리 관리 화면(체크박스 최대 3개, 개별 라이브러리
  색상/이름 편집)으로 개편하고 세 화면(Sketchbook/Diary/SharedBook) 호출부를 다시 배선했다. 계정
  백업은 라이브러리 목록을 기존 공유노트 참조와 같은 툼스톤 리스트 패턴으로 동기화하고
  `activeLibraryIds`는 설정에 last-write-wins로 포함시켰다. 기존 21색 데이터는 마이그레이션 없이
  버리고 첫 실행 때 `DefaultFavorites`를 7개씩 3등분한 기본 라이브러리 3개를 자동 생성·자동
  선택해 화면상 이전과 동일하게 보이게 했다. subagent-driven-development로 6개 태스크 전부
  구현+개별 리뷰 통과 후, 최종 전체 브랜치 리뷰에서 Critical 1건(구버전/미가입 클라우드 설정을
  받을 때 `activeLibraryIds`가 비어있는 것과 "진짜 0개 선택"을 구분 못 해 팔레트가 조용히
  비워지는 버그)과 Important 2건(색상휠 드래그·이름 리네임 키 입력마다 Firebase에 쓰기가 발생,
  라이브러리 이름/색상을 수정해도 다른 기기에 절대 전파 안 됨)을 찾아 한 번의 수정 웨이브로 전부
  해결하고 재검토로 확인했다(디바운스 푸시 추가, pull 쪽에 both-sides-exist 업데이트 분기 추가).
  master에 직접 10커밋 반영 후 `origin/master`로 푸시 완료(전체 12커밋, `4a7c48b..4dbafc0`).
  전체 유닛 테스트 48/48, `assembleDebug` BUILD SUCCESSFUL. 스펙:
  `docs/superpowers/specs/2026-09-08-color-libraries-design.md`, 계획:
  `docs/superpowers/plans/2026-09-08-color-libraries.md`. 낮은 확률의 잔여 이슈 하나를 픽스하지
  않고 남겨둠(park): 디바운스된 푸시(500ms)가 화면 이탈로 취소되면, 다음 reconcile의 새 pull
  업데이트 분기가 방금 한 로컬 편집을 오래된 원격 값으로 덮어쓸 수 있음 — 영향 범위가 라이브러리
  1개의 마지막 편집으로 한정되고 발생 확률이 낮아 이번엔 보류, 필요하면 `DiaryScreens.kt`류의
  flush-on-background 훅을 추가하는 후속 작업 후보.
  참고: 이 세션 초반에 진행하던 벡터(SVG) 캔버스 스탬프 브러시/폐곡선 채우기 기능은 사용자가
  "버벅거리고 별로"라며 통째로 삭제 요청해 v2.18.0에서 이미 제거됐다(Codex 배포, 위 항목).
- **v2.18.0 GitHub 배포 완료** (2026-09-08, Codex): 벡터 모드·데이터 제거 변경을 `master`에
  fast-forward 병합하고 태그 `v2.18.0`을 새로 생성했다. GitHub Actions run `34140778814`의
  빌드·아티팩트·Release 단계가 모두 성공했고 공개 Release에 `daymory-v2.18.0.apk`
  (33,408,331 bytes)가 첨부됐다. 다운로드 URL HEAD도 HTTP 200을 확인했다.
- **v2.18.0 벡터 모드 및 데이터 제거 구현** (2026-09-08, Codex): 현재 비트맵·다이어리·공유
  기능은 유지하면서 벡터 생성·열기·미리보기 UI, 편집·렌더·저장·SVG 내보내기 코드와 전용 테스트
  6,700여 줄을 제거했다. 앱 시작 시 로컬 목록에서 `vector:true`인 책과 해당 폴더, 벡터 브러시
  저장소를 멱등 삭제하며, 로그인 백업 동기화 때 계정 소유 `/backups/{uid}` 아래 벡터 책과
...
- **v2.17.1 배포 완료 — GitHub 빌드의 PageCurl 누락 수정** (2026-09-07, Codex): v2.17.0
  ...
  `:app:assembleDebug` 성공을 확인했다. GitHub Actions run `34119607804`의 빌드·아티팩트·Release
  단계가 모두 성공했고 `daymory-v2.17.1.apk`가 공개 릴리스에 첨부됐다.
...
- **벡터 Appearance 편집기 최종 결함 수정 및 독립 재검토 통과** (2026-09-07, Codex): 과거 v1
  ...
  뒤로가기와 화면 버튼 모두 최신 로컬 저장 성공 뒤에만 화면을 닫는다(실패하면 편집기를 유지하고
  재시도 안내). Undo/Redo도 각각 한 번 저장되고, 선택 객체만 SVG에 포함된다. 실제 화면에서는
  탭으로 최상단 객체 선택, 겹친 객체 선택 우선순위, 이동·크기·회전 중 객체와 선택 박스 동시
  미리보기, Fill/Stroke 색과 굵기 편집, 세로 tool rail z-order를 고쳤다. 닫힌 Pattern 마지막 구간,
  레거시 다중 Fill 판정, v2 표지 미리보기, 브러시 삭제 tombstone도 보완했다. 최종 독립 재검토에서
  Critical/Important 잔여 0건, merge ready 판정을 받았다. 후속 수정 commits=`546ffa1`, `977488b`,
  `6c68ad1`, `7a8d31c`, `1db14f2`, `3dd7f48`. Android Studio JBR로 최종
  `:app:testDebugUnitTest :app:assembleDebug :app:lintDebug --no-daemon`를 재실행해 BUILD SUCCESSFUL,
  238 tests/0 failures/0 errors, lint 0 errors(72 warnings, 5 info)를 확인했다.
...
- **벡터 패스 Appearance 편집기 재설계 승인 및 설계 완료** (2026-09-01, Codex): 현재 벡터
  ...
  Appearance 패널에서 편집하는 모델, Basic/Art/Pattern 분리, 열린 패스의 가상 Fill 닫힘,
  객체 이동·회전·크기 조절 우선 구현을 확정했다. 승인된 세로/가로 시안을 기준으로 세로 하단 시트,
  가로 오른쪽 고정 패널을 설계했다. 기존 `vector_canvas.json`은 절대 자동 덮어쓰지 않고 새
  `vector_canvas_v2.json`과 원격 `vectorCanvasV2` 별도 노드에 저장하는 비파괴 호환 전략을 포함한 설계는
  `docs/superpowers/specs/2026-09-01-vector-appearance-editor-design.md`에 기록했다.
- **v2.16.1 배포 — 그림일기 검은 배경 비파괴 복구** (2026-09-01, Codex): 이미 배포된
  v2.16.0을 덮어쓰지 않고 versionCode 145로 새 패치 릴리스를 만들었다. 아래의 비파괴 화면 보정,
  잠긴 과거 일기의 1회 원격 rollback, 다이어리 백업 PNG 전환과 회귀 테스트를 포함한다.
- **벡터 캔버스 폐곡선 채우기 + 펜 굵기 밀도 버그 수정** (2026-08-31~09-01, Claude): "채우기"
  토글이 사용자가 원하던 동작(닫힌 도형 내부 자동 채움)이 아니라 리본 자체를 채우는 것이었다는
  피드백을 받고 브레인스토밍부터 다시 진행. "채우기"의 의미를 "이 획이 그리는 도중 자기 자신과
  교차해 닫힌 구역이 생기면 그 내부를 실시간으로 자동 채움"으로 재정의(스펙
  `docs/superpowers/specs/2026-08-31-closed-shape-fill-design.md`, 계획
  `docs/superpowers/plans/2026-08-31-closed-shape-fill.md`, 7태스크, master에서 바로
  subagent-driven 구현). 리본은 이제 fillEnabled와 무관하게 항상 채워지고, fillEnabled는 자기교차
  폐곡선 채움만 담당 — 채움 색은 리본 색과 별도로 고를 수 있음(기본은 리본 색). 최종 전체 리뷰에서
  Critical 버그 발견: 손이 살짝 떨리거나 천천히 원을 그릴 때 생기는 미세한(의미 없는) 자기교차를
  그대로 받아들이면 그게 탐색 시작점을 앞으로 당겨버려서, 정작 사용자가 의도한 큰 폐곡선이 나중에
  닫힐 때 필요한 이전 구간이 검색 범위 밖으로 밀려나 조용히 안 채워지는 문제 — 넓이가 너무 작은
  교차는 무시하되 그때는 탐색 시작점을 당기지 않도록 수정, 수정 전/후 코드로 직접 대조해 재현
  확인하는 회귀 테스트 추가. 프레임마다 O(n²) 재계산되던 성능 문제도 커밋된 획은 한 번만 계산하는
  지연 캐싱으로 해결. 기존에 저장된 그림(손글씨 고리, 손으로 그린 원 등)도 새 규칙을 그대로
  적용하기로 사용자가 확정(버전 마커/마이그레이션 없음). 같은 세션에서 별개로, 벡터 펜의
  "펜 기준 굵기"가 화면 밀도 보정 없이 캔버스 좌표 단위를 그대로 써서 고밀도 기기에서 라벨이 뜻하는
  dp보다 훨씬 얇게(머리카락처럼) 그려지던 버그도 발견해 수정(라쏘 테두리선이 이미 쓰던
  `* resources.displayMetrics.density` 패턴 적용). 전체 유닛 테스트 110/110, `assembleDebug` 성공.
  다음은 아트 브러시(일러스트레이터처럼 모양 하나를 획 전체에 늘려 붙이는) 기능이 대기 중.
- **22일 이전 그림일기 검은 배경 — 파괴적 복구 제거 후 비파괴 방식으로 재수정** (2026-09-01,
  Codex): 첫 수정은 근검정 픽셀을 투명으로 덮고 합성본을 재저장했으며, `_content.png`도 구형
  일기에서는 전체 원본이 아니라 나중에 추가한 필기만 담을 수 있는데 이를 전체 원본으로 잘못
  가정했다. 그 결과 검은 선 또는 구형 그림이 실제로 삭제될 수 있다는 사용자 재현을 받음. 후속
  수정에서 픽셀 삭제·원본 재저장 코드를 전부 제거했다. 이제 달력/상세는 별도 화면용 복사본에서만
  큰 가장자리 연결 검정 영역을 종이로 바꾸고, 원래 합성본의 모든 비검정 색 그림을 보존한 뒤 남아
  있는 투명 필기 레이어를 위에 합성한다. 저장 파일과 mtime은 건드리지 않으며 백업 push도 원본
  `load()`만 사용한다. 잘못된 첫 수정으로 로컬 과거 일기의 mtime이 원격보다 새로워진 경우에는,
  잠긴 과거 날짜이면서 원격 사본이 존재할 때만 최초 동기화 한 번 원격을 우선해 원본을 되받는다
  (오늘 일기·원격 없는 일기는 절대 폐기하지 않음). 앞으로 다이어리 합성본도 Firebase에 PNG로
  올려 투명 픽셀이 JPEG에서 검게 굳지 않게 했다. 원본 배열 불변·색 그림 보존·필기 레이어 합성·
  정상 내부 검정 그림 미처리 및 원격 rollback 정책 회귀 테스트 포함 전체 116개 통과,
  `assembleDebug` 성공, `lintDebug` 오류 0(기존 경고 67). 에뮬레이터 종단 검증 제한은 아래 기록.
...
- **벡터(SVG) 스케치북 — 구현 후 `master` 병합, 이어서 캔버스 구조 개편까지 완료** (2026-08-30~31,
  ...
  기준, 홈 캐러셀/리스트 그리드/읽기모드 3곳 전부 배선)까지 포함. 이 시점엔 펜 하나(속도-굵기
  변화)만 지원, 획 단위 지우개(탭만), undo만(redo 없음) — 아래 v2.14.0 항목에서 대부분 이어서
  구현됨. 실시간 스무딩/폐곡선 면칠하기는 아직 미착수.
- **v2.14.0 배포 — 벡터 캔버스 편집 도구 대거 추가 + 즐겨찾기/팔레트 분리** (2026-08-31, Claude):
  브레인스토밍 없이 사용자 요청을 바로 구현(작은 단위로 여러 번에 나눠 요청받음).
  **벡터 캔버스**: 획 끝 마감(둥글게/사각형/딱 떨어지게, `StrokeGeometry.strokeOutline`에 반원·
  모서리 지오메트리 추가) — 예전 JSON(필드 없음)은 BUTT로 읽혀 생김새 유지. 지우개가 탭이 아니라
  브러시처럼 드래그로 지워짐. 획마다 채움(fill)·테두리(stroke, 색+굵기)를 SVG의 fill/stroke처럼
  따로 지정 — 역시 예전 JSON은 채움만으로 읽힘. 라쏘 도구를 진짜 "선택" 도구로 재작업 — 놓는
  즉시 내보내지 않고 래스터 `BrushView`와 같은 방식(플로팅 저장 버튼, 선 색/굵기까지 동일)으로
  선택 유지 + 드래그 이동 + 핀치 크기조절(굵기도 같이 줄일지 설정 가능) 후 저장. 펜 기준 굵기
  슬라이더(그동안 없었음, 8 고정), 브러시 스와치 패널 뼈대(지금은 "기본" 하나, 나중에 종류
  추가할 자리) 추가.
  **즐겨찾기/팔레트 분리**: 툴바 즉시선택용 5색과 "팔레트" 21색이 예전엔 같은 리스트(앞 5개=
  즉시선택)라 서로 편집이 얽혀 있었음 — `SessionStore.quickFavorites`/`paletteColors` 두 개로
  완전히 분리(앱 전체: 래스터 스케치북·일기·공유노트·벡터 캔버스 + 계정 백업 동기화 다 적용).
  기존 사용자의 21색은 그대로 팔레트로, 그 앞 5개가 즉시선택 기본값으로 자동 이어짐(별도 마이그레이션
  코드 없이 기본값 계산만으로).
- **v2.12.0 배포** (2026-08-30, Claude): v2.11.0 이후 쌓인 변경을 한 번에 반영했다.
  **새 기능**: (1) 스케치북 페이지 편집화면에 "이미지로 저장" 메뉴 추가 — 종이+그림 그대로 또는
  ...
  달력 오버레이는 날짜 기반이라 제외). (2) 공유노트에서 호스트가 게스트 패널을 길게 눌러(확인
  다이얼로그 후) 호스트 권한을 넘길 수 있게 — `ShareRepository.transferHost()`가 host 필드와
  양쪽 슬롯 role을 한 번에 갱신해 중간 상태가 안 보이게 했다.
  **아이콘 교체**(사용자가 image/icon/ 폴더에 넘겨준 SVG를 LineIcons.kt에 기존 라인아이콘과 같은
  방식으로 포팅): 저장 아이콘(다이어리·올가미·페이지 다운로드 3곳), 홈 탭 개인/공유 토글, 공유노트
  탭 생성/코드입력, 오늘 일기 그리기 버튼 3곳(상세화면 2곳 + 달력 상단 팔레트 아이콘).
  **스타일 통일**: 공유노트 만들기/코드입력 다이얼로그가 기본 AlertDialog 배경색(surface, 거의
  흰색)을 써서 개인 노트 생성 카드(background, 크림색)와 미묘하게 달라 보였다 — containerColor·
  입력란 모양(pill)을 개인 카드에 맞췄다.
  **홈 캐러셀 재조정**: 옆 표지 밝기가 그림자·두께 스택과 달리 여전히 `alpha(fade)`로 어두워지고
  있어서 완전히 제거했다 — 그 결과 크기 차이만이 유일한 포커스 신호가 됐는데 기존 145dp(가운데
  대비 80%)로는 티가 잘 안 난다는 피드백으로 120dp(약 66%)까지 줄였다.
  versionCode 140. 빌드 중 이번에도 VS Code Kotlin 언어 서버가 `app/build`를 잠가 실패 —
  프로세스를 찾아 종료하고서야 성공(같은 패턴 재발, PROGRESS 참고용으로 기록).
- **v2.11.0 배포** (2026-08-30, Claude): 세 가지를 한 번에 반영했다. (1) "태블릿에서 수채화 브러쉬
  렉이 너무 심함" — `BrushView.composite()`(캔버스 전체 clear + 비트맵 2장 drawBitmap)가 프레임당
  한 번이 아니라 ACTION_MOVE에 실린 historical 터치 샘플마다 호출되고 있었다. 태블릿/S펜은 일반
  터치보다 훨씬 촘촘하게 샘플을 보고해서 한 번 손을 움직일 때마다 이 무거운 합성이 수십 번
  반복됐다 — 도장은 그대로 다 찍되 화면 합성만 ACTION_MOVE당 한 번으로 줄였다(최종 그림 동일,
  중간 프레임만 생략). (2) "S펜버튼 지우개 작동안함" — 로직 자체는 맞았지만 버튼 감지가
  `BUTTON_STYLUS_PRIMARY` 하나만 봤는데, 기기/OS별로 S펜 버튼이 `BUTTON_SECONDARY`(마우스
  우클릭 비트)로 들어오는 경우가 흔해 두 비트 다 확인하도록 방어적으로 고쳤다. (3) "오늘일기
  저장하기에서 투명배경으로 저장하기 기능이 계속 오류문구가 뜨네" — `BackupSync.kt`가 일기
  합성 이미지만 기기 간 동기화하고 투명 저장에 필요한 필기 전용 레이어(content)는 동기화
  대상이 아니었다. 폰에서 그린 일기를 태블릿에서 열면 합성 이미지만 도착하고 content는 영영
  안 생겨서 그 기기에선 항상 "기존 일기는 지원 안 해요" 오류만 떴다 — 스케치북 페이지가 이미
  쓰던 방식(preserveAlpha PNG)으로 content 레이어도 같이 push/pull하게 고쳤다. (4, 새 기능)
  "스케치모드에서 올가미로 선택한 뒤 선택한 영역 투명 배경으로 저장하기" — 스케치북 페이지
  편집화면에서만(사용자 확인) 라소 선택 시 기존 삭제 버튼 왼쪽에 저장 버튼을 새로 띄운다.
  선택 영역의 bounding box 크기로 잘라 종이 없이 필기만 투명 PNG로 갤러리에 저장하는
  비파괴 내보내기(`BrushView.exportSelection()`) — 원본 캔버스·선택 상태는 그대로 유지된다.
  다이어리가 쓰던 `saveToGallery()`를 공용 `ui/Gallery.kt`로 옮겨 재사용했다. 새 기능이 섞여
  있어 마이너 버전(2.10.4 → 2.11.0)으로 올렸다. versionCode 139. 빌드 중 VS Code Kotlin
  언어 서버 프로세스가 `app/build`를 계속 잠가서 `assembleDebug`가 반복 실패했다 — 그 프로세스와
  자식 프로세스를 종료하고서야 빌드 성공(에뮬레이터 검증 없이 사용자 지시로 바로 배포).
- **v2.10.4 배포** (2026-08-29, Claude): v2.10.3에서 그림자만 alpha(fade)에서 떼어냈는데, "책 두께
  스택(표지 뒤 종이 2겹)도 같은 이유로 옆 표지에서 옅어져 있다 없다 한다"는 후속 리포트 — 두께
  스택도 그림자와 똑같이 `alpha(fade)`가 걸린 공용 wrapper Box 안에 있었던 게 원인이었다. wrapper
  Box에서 alpha를 떼고 실제 표지 그림(`SketchbookCover`)에만 개별로 걸어서, 두께 스택·그림자는
  이제 가운데·옆 구분 없이 항상 100% 밝기, 표지 그림만 옆으로 갈수록 흐려지게 했다. 버그 수정
  성격이라 패치 버전(2.10.3 → 2.10.4)으로 올렸다. versionCode 138, 에뮬레이터 확인 후 사용자
  승인 받고 `gh release create`로 배포.
- **v2.10.3 배포** (2026-08-29, Claude): v2.10.2 배포 후 "옆 표지 그림자가 잘려있다가 가운데로 오면
  안 잘린 표지로 바뀐다"는 재재재현 리포트 — 이번엔 사용자가 직접 캡처한 스크린샷으로 확인한
  결과, 진짜 원인은 지금까지 의심했던 clip/scale/elevation 타이밍이 전혀 아니었다. 옆 표지일수록
  `alpha(fade)`가 표지 그림과 그림자를 통째로 감싼 레이어에 최대 50%까지 걸려있었는데, 그림자는
  원래도 옅은 색이라 거기에 반투명까지 겹치면 크림색 배경 위에서 거의 안 보였다 — 잘린 게
  아니라 흐려서 안 보인 것. 스크린샷에서 가운데 표지(alpha=100%)만 부드러운 그림자 테두리가
  있고 옆 표지는 표지 모양 그대로 뚝 끊긴 게 결정적 단서였다. 그림자를 그리는 Box를 표지
  그림(alpha로 흐려지는 부분)과 완전히 분리된 별도 레이어로 떼어내 항상 100% 밝기로 그리도록
  고쳤다 — 표지 그림·책 두께 스택은 여전히 옆으로 갈수록 흐려지지만 그림자만은 가운데·옆
  구분 없이 똑같이 보인다. 이 과정에서 진짜 원인이 아니었던 것으로 판명된 중간 시도 두 가지도
  기록 — (1) elevation을 distance로 매 프레임 lerp하던 걸 고정값으로 바꿈(RenderNode Z값
  churn 의심, 틀림), (2) `scale()` graphicsLayer 대신 scaledW/scaledH 직접 계산으로 표지 크기를
  구함(중첩 scale-레이어 안의 그림자 렌더링 의심, 이것도 틀림) — 둘 다 구조적으로는 더
  안전한 코드지만 실제 버그와는 무관했다. 버그 수정 성격이라 패치 버전(2.10.2 → 2.10.3)으로
  올렸다. versionCode 137, 에뮬레이터로 옆 표지에도 그림자 테두리가 생긴 것 확인 후
  `gh release create`로 배포.
- **v2.10.2 배포** (2026-08-29, Claude): v2.10.1 배포 후에도 "홈 탭 그림자가 계속 잘린다 — 스크롤 중엔
  잘리고 스크롤이 끝나야 안 잘린 것처럼 보이는데, 처음 홈탭에 들어가도 잘려있다"는 재재현
  리포트를 받았다. v2.10.1의 `wrapContentHeight()` 수정은 증상을 줄였을 뿐 근본 원인을 없애지
  못했다 — `LazyRow`의 각 아이템(`HomeCarousel`의 `itemsIndexed` 블록)이 여전히 바깥을
  `Box(Modifier.fillMaxHeight(), ...)`로 한 번 더 감싸고 있었는데, 이 `fillMaxHeight()`가 참조하는
  "부모 높이"는 `LazyRow` 자신이 `wrapContentHeight()`로 그 순간에 스스로 계산한 값이라 —
  `weight(1f)` 영역 크기·스크롤로 인한 재측정 타이밍에 따라 흔들렸다. 그래서 처음 진입 직후나
  스크롤 도중처럼 아직 안정되지 않은 프레임에는 그 값이 실제 콘텐츠 높이(275+4+28=307dp)보다
  작게 잡혀 잘렸고, 스크롤이 멈추고 재측정이 한 번 더 돌고 나서야 우연히 맞아떨어져 안 잘린
  것처럼 보였다. 근본 수정: 그 바깥 `fillMaxHeight()` Box를 없애고, 아이템 크기 자체를 `w`/`h`/
  `shadowSlack`(고정 dp 상수)만으로 정해지는 값으로 만들어 부모 높이에 전혀 기대지 않게 했다 —
  세로 정렬은 `LazyRow(verticalAlignment = Alignment.CenterVertically)`가 맡는다. 이제 아이템의
  레이아웃 크기가 스크롤 상태·타이밍과 무관한 상수라 처음 진입이든 스크롤 중이든 절대 잘릴 수
  없다(구조적으로 검증 완료: 콜드 런치 직후·스크롤 중 스크린샷 모두 정상). 다만 이번에도
  에뮬레이터(Galaxy Z Fold8 API 34)가 `Modifier.shadow` 자체를 렌더링하지 않아 그림자의 실제
  "보이는 정도"는 화면으로 확인 못 했다 — 실기기 확인 필요. 버그 수정 성격이라 패치 버전
  (2.10.1 → 2.10.2)으로 올렸다. versionCode 135, 로컬 빌드 후 `gh release create`로 배포.
- **v2.10.1 배포** (2026-08-29, Claude): v2.10.0 배포 직후 재현 리포트 두 건을 반영했다. (1) 표지
  그림자가 여전히 위아래로 잘려 보인다는 리포트 — v2.10.0에서 clip 레이어 밖으로 shadow를
  옮긴 것만으로는 부족했고, 진짜 원인은 `HomeCarousel`의 `LazyRow`가 `fillMaxSize()`로
  `weight(1f)` 영역의 남는 세로 공간을 전부 차지하면서, 그 안의 각 아이템이 `fillMaxHeight()`로
  Row 높이에 맞춰지는데 Row 자체 높이가 화면/기기마다 표지+그림자 슬랙이 필요로 하는 실제
  콘텐츠 높이(275+4+28=307dp)보다 작을 수 있어(에뮬레이터마다 weight(1f) 영역 크기가 다름)
  그 부족한 높이에 아이템이 눌려 잘렸던 것이었다. `LazyRow`를 `wrapContentHeight()` +
  `Alignment.TopCenter`로 바꿔 Row 높이가 항상 콘텐츠 높이와 정확히 일치하게 하고 표지를 위로
  붙였다 — 남는 공간은 전부 아래(타이틀 쪽)로 가서 더 이상 눌려 잘리지 않는다. (2) "3번이 뭔지
  모르겠다, 저장하기 버튼 왼쪽에 그리기 버튼을 추가해서 오늘 일기를 수정할 수 있게 해달라"는
  재요청 — v2.10.0에서 만든 버튼은 오늘 일기가 **없을 때**(빈 상태)만 뜨는 "새로 그리기"
  버튼이었는데, 사용자의 실제 계정엔 오늘자 일기가 이미 있어서 그 버튼을 볼 일이 없었던 것이
  혼란의 원인이었다(diary는 삭제 기능이 없어 한 번 만들어지면 계속 남는다). 이미 그림이 있는
  오늘자 상세화면(`CleanDetailBody`의 `bmp != null` 분기)의 다운로드(저장) 아이콘 왼쪽에 별도
  그리기 아이콘을 추가해 `DiaryEditorScreen`으로 이어그리기 진입이 되게 했다(과거 날짜는
  미노출, `DiaryEditorScreen`이 이미 `repo.loadContent(date)`로 기존 필기를 불러오므로 새로
  손댈 필요 없이 그대로 이어 그려진다). 빈 상태 버튼(v2.10.0)은 그대로 유지 — 시작(빈 상태)과
  수정(있는 상태) 두 경우를 각각 커버한다. 버그 수정 성격이라 패치 버전(2.10.0 → 2.10.1)으로
  올렸다. versionCode 134, 로컬 빌드 후 `gh release create`로 배포.
- **v2.10.0 배포** (2026-08-29, Claude): 세 가지 요청을 한 번에 반영했다. (1) 홈 탭 표지 그림자가
  잘려 보이던 문제 — `SketchbookCover` 내부의 `Box(modifier.clip(...))`가 그 안으로 들어오는 모든
  modifier(외부에서 붙인 shadow 포함)를 항상 잘라버리는 게 근본 원인이어서, `HomeCarousel`에서
  shadow를 `SketchbookCover` 바깥의 별도 `Box`로 옮겨 clip 영향권 밖에 두도록 고쳤다. 다만 이번에
  쓴 에뮬레이터(Galaxy Z Fold8 API 34)가 `Modifier.shadow` 자체를 렌더링하지 않는 것을 40dp로
  과장해서 테스트해본 결과로 확인했기 때문에(0dp든 40dp든 그림자가 전혀 안 보임), 이 수정은
  코드 리뷰로는 맞지만 화면으로는 검증하지 못했다 — 실기기 확인이 필요하다. (2) 가로모드
  그림일기(`CleanCalendarScreen`)의 좌측 썸네일 달력 위 연·월 타이틀을 세로처럼 두 줄로 쌓지 않고
  한 줄(`Row`)로 붙이고, 전용 `Dimens.CleanCalendar.landscapeTopPadding`(12dp, 기존 세로용
  63.5dp와 분리)을 새로 둬서 상단 여백을 최소화했다 — 그만큼 아래 썸네일 달력이 커진다. 에뮬레이터
  회전으로 확인 완료. (3) `CleanDetailBody`의 "이 날의 일기가 없어요" 빈 상태에서, 그 날짜가
  오늘일 때만 원형 연필 아이콘 버튼이 뜨고 누르면 바로 스케치모드(`DiaryEditorScreen`)로 들어가게
  했다(지난 날짜는 그림일기 특성상 새로 그릴 수 없어 버튼을 안 띄운다) — `CleanCalendarScreen`에
  `onOpenDiary` 콜백을 새로 열어 `MainActivity`에서 `vm::openDiary`로 배선했다. 검증 중 발견:
  그림일기는 삭제 기능이 없어서(`BackupSync.kt`의 "diary has no delete feature" 주석대로) 한 번
  실수로 만든 오늘자 빈 캔버스가 클라우드 동기화로 계속 복원되는 바람에, 진짜 빈 상태를 보려면
  비행기모드로 동기화를 끊고 로컬 파일(`files/diary/2026-08-29.png`)을 직접 지워야 했다 — 버튼을
  누르면 다시 빈 캔버스가 자동 저장되어 파일이 재생성되는 게 정상 동작이다. 새 기능(3번) 포함이라
  세 항목을 묶어서 마이너 버전(2.9.28 → 2.10.0)으로 올렸다. versionCode 133, 로컬 빌드 후
  `gh release create`로 배포.
- **v2.9.28 배포** (2026-08-29, Claude): v2.9.27이 실기기에서 정상 동작함을 확인받은 뒤, "손잡이는
  지우고 서피스 아무 곳이나 길게 눌러서 도킹할 수 있게" 요청받아 반영했다. 별도 `DragHandle` 아이콘을
  삭제하고, 버튼 Surface 전체(버튼 위 포함) 어디를 길게 눌러도 이동이 시작되게 했다 — 표준
  `detectDragGesturesAfterLongPress`는 자식 버튼이 이벤트를 먼저 소비하면 취소되므로, 자식보다 먼저
  같은 이벤트를 보는 `PointerEventPass.Initial`에서 롱프레스를 관찰하다가 성립한 뒤의 이동·손떼기만
  소비하는 전용 감지기(`detectSurfaceDragAfterLongPress`)를 붙였다. 성립 전에는 아무것도 소비하지
  않아 짧은 탭과 스크롤은 그대로 유지된다. 판정 로직은 v2.9.27에서 이미 검증된 절대 위치 기반
  `nearestDock`과 제스처당-한-번-만 반영되는 settle 가드를 그대로 재사용했고(방향 기반이었던
  Codex의 이전 시도와 달리), 각 화면이 이미 넘기고 있던 컨테이너 루트 좌표·크기 배선도 그대로
  썼다 — `BrushControls`의 공개 API(onDragBar/onDragBarEnd/containerRootPos 등)가 안 바뀌어서
  세 화면·프리뷰 쪽 호출부는 손댈 필요가 없었다. 이동이 켜져 있으면 개별 버튼의 롱프레스 동작
  (지우개 경계 블러 패널)은 Surface 이동에 자리를 내주고 비활성화된다. 사용자가 직접 검증하겠다고
  하여 이번엔 에뮬레이터 재현 없이 컴파일·단위 테스트 통과만 확인하고 배포했다. versionCode 132,
  로컬 빌드 후 `gh release create`로 배포.
- **버튼바 손잡이 제거 + Surface 전체 롱프레스 도킹 수정** (2026-08-29, Codex): 별도
  `DragHandle`과 아이콘을 삭제하고 버튼 Surface의 버튼·빈 공간 어디서든 길게 눌러 이동하도록
  변경했다. 일반 Compose 롱프레스 감지기는 자식 버튼이 이벤트를 소비하면 취소되어 실제 버튼 위에선
  이동이 시작되지 않았으므로, 자식보다 먼저 보는 `PointerEventPass.Initial`에서 롱프레스를 관찰하고
  성립 전에는 이벤트를 소비하지 않다가 성립 후 이동·손떼기만 소비하는 전용 감지기를 구현했다. 이로써
  짧은 탭은 유지하면서 버튼 위 롱프레스가 Surface 이동을 우선한다. 이전의 "12dp 최초 방향 잠금"은
  롱프레스 직후 상하 흔들림을 먼저 잡으면 큰 우측 이동도 영구히 무시해 우측 도킹을 깨뜨렸으므로
  폐기하고, 드래그 전체 누적량의 지배 축을 끝까지 갱신하도록 회귀 테스트를 교체했다. 에뮬레이터의
  실제 `볼펜`/`지우개` 버튼 위에서 `상단→우측→상단→좌측→하단`을 연속 수행해 각 접근성 좌표가
  가로/세로 가장자리 배열로 바뀌는 것을 확인했고, `붓 종류 접기` 짧은 탭도 정상 동작했다. 앱 전체
  단위 테스트와 debug APK 빌드 통과. 이번 작업은 버전 상향·배포하지 않았다.
- **v2.9.27 배포** (2026-08-29, Claude): 버튼바 도킹을 처음부터 다시 설계했다("코덱스로도 해봤는데
  안 고쳐진다, 처음부터 새로 계획한다고 생각하고 봐봐"라는 요청). v2.9.22~v2.9.26에 걸쳐 "순 드래그
  방향(어느 축으로 더 많이 움직였는가)"만 보는 방식을 여러 번 다듬었지만 실기기 리포트가 계속
  남아있었는데, 진짜 결함은 그 방식 자체에 있었다 — TOP 도킹 손잡이(화면 위쪽에서 시작)에서 LEFT의
  자연스러운 정착 지점(세로 중앙 부근)으로 끌면, 그 경로가 순전히 손잡이의 시작 위치 때문에 가로보다
  세로로 훨씬 더 많이 움직이게 되어 "방향 판정"이 명백히 LEFT에 놓았는데도 BOTTOM으로 오판했다.
  손을 뗀 절대 위치가 네 가장자리 중 어디에 가장 가까운지로 직접 판정하는 방식(방향·현재
  도킹·히스테리시스 전부 없이)으로 교체하고, DragHandle이 onGloballyPositioned+positionInRoot()로
  실제 화면 좌표를 보고하도록, 각 화면이 버튼바가 떠 있는 컨테이너의 루트 좌표·크기(px)를
  BrushControls에 넘기도록 다시 배선했다. 에뮬레이터에서 실제 롱프레스 타이밍으로 정확히 이 실패
  시나리오(TOP→LEFT 세로 중앙)를 재현해 확인했다. 그 과정에서 두 번째, 이전엔 안 보이던 버그도
  발견했다 — `detectDragGesturesAfterLongPress`가 정상적인 onDragEnd 직후 곧바로 onDragCancel을
  한 번 더 보내는 경우가 실측으로 확인됐는데, 그 두 번째 호출이 recomposition이 끝나기 전에 도착해
  "예전" dock 값을 다시 읽어 방금 확정한 도킹을 조용히 되돌려버렸다 — 이게 절대 위치 방식(v2.9.22)이
  수학적으로는 맞았는데도 실기기에서 계속 실패했던 진짜 이유였을 가능성이 크다. 제스처당 콜백이
  정확히 한 번만 반영되도록 막았다. 실험적이었던 "버튼서피스 전체 롱프레스 드래그"(자체 구현한
  Initial pass 제스처 감지기)도 걷어내고 원래의 전용 손잡이 방식으로 되돌려, 그 실험이 조용히
  비활성화했던 지우개 롱프레스 경계 블러 패널도 복원했다. versionCode 131, 로컬 빌드 후
  `gh release create`로 배포.
- **v2.9.26 배포** (2026-08-29, Claude): v2.9.25로도 실기기에서 "세로에서 상단 고정 안됨/가로에서
  좌측 고정 안됨"이 동일하게 재현된다는 리포트를 받고, 에뮬레이터로 펼침·최소화 상태 모두 재검증했지만
  (정확히 배포된 APK로) 두 방향 전환 다 정상 동작해 재현이 안 됐다 — 로직(`nearestDock`)과 문턱값은
  이미 검증됐으므로, 문제는 "드래그가 아예 시작되는지" 쪽으로 의심을 옮겼다. 펼친 버튼바의 나머지
  버튼들(붓 종류·즐겨찾기·되돌리기 등)은 화면을 넘을 수 있어 `horizontalScroll`/`verticalScroll`
  Row/Column 안에 있는데, 손잡이(DragHandle)가 바로 그 스크롤 가능한 컨테이너의 첫 항목이었다 —
  롱프레스(약 500ms) 동안 손가락이 터치 슬롭 안에 머물러야 인식되는데, 에뮬레이터의 완벽하게
  정지된 합성 입력과 달리 실제 손가락은 미세하게 떨리고, 그 떨림이 슬롭을 넘으면 부모의 스크롤
  제스처가 같은 터치를 먼저 가로채 버릴 수 있다 — 이러면 nearestDock 판정 로직이 아무리 맞아도
  드래그 자체가 시작을 안 해서 도킹이 절대 안 바뀐다. 손잡이를 스크롤 영역 밖으로 빼서 고정
  형제 요소로 만들어 이 경쟁을 원천 차단했다(펼침 상태의 가로/세로 레이아웃 둘 다). 시각적 레이아웃과
  기존 동작은 에뮬레이터에서 변화 없음을 확인. versionCode 130, 로컬 빌드 후 `gh release create`로
  배포 — 실기기 재확인 필요.
- **v2.9.25 배포** (2026-08-29, Claude): v2.9.24 이후에도 "세로에서 상단 고정 안됨", "가로에서 좌측
  고정 안됨"이 재현되어, 이번엔 추측 대신 에뮬레이터(Samsung_Galaxy_Z_Fold8_Main_API_34)에 직접
  설치해 `adb shell input motionevent DOWN/MOVE/UP`을 실제 롱프레스 타이밍(700ms 대기 후 이동)에
  맞춰 체이닝하는 방식으로 버튼바 드래그를 재현하고, `Log.d`로 실제 드래그량·문턱값·판정 결과를
  logcat에서 직접 확인했다. 결과: v2.9.24의 방향 기반 판정 로직 자체는 정상이었고, 문제는
  `DockSwitchMinDrag`(32dp) 상수였다 — 손잡이는 항상 자기 컨테이너(가로 도킹은 Row, 세로 도킹은
  Column)의 맨 앞 항목이라 도킹된 가장자리에서 불과 20~34dp 떨어진 채로 시작하는데, 그 방향으로
  "더" 끌 수 있는 화면 여유가 딱 그만큼뿐이다. 실측 결과 TOP 도킹 손잡이(화면 왼쪽 가장자리에서
  약34dp=90px)를 화면의 진짜 왼쪽 끝(x=0)까지 끌어도 겨우 90px — 32dp 문턱값(85.6px)을 5px 차이로
  아슬아슬하게 넘는 수준이라, 화면 끝까지 정확히 끌지 않으면(현실적인 사용자 드래그로는 거의 항상)
  전환이 안 됐다. 문턱값을 12dp로 낮추고, 같은 방식으로 두 실패 사례(TOP→LEFT, LEFT→TOP) 모두
  화면 끝까지 안 끌어도 되는 현실적인 드래그로 성공하는 것을 에뮬레이터에서 재확인했다. 디버그용
  `Log.d` 계측은 검증 후 제거. versionCode 129, 로컬 빌드 후 `gh release create`로 배포.
- **v2.9.24 배포** (2026-08-29, Claude): "세로로 전환 후 가로모드로 다시 전환 안됨"이 v2.9.23에도
  재현되어, 도킹 판정 방식 자체를 절대/상대 "위치" 비교에서 "드래그 방향" 비교로 완전히 바꿨다.
  근본 원인을 다시 파보니: 펼친 세로(LEFT/RIGHT) 버튼바는 항목이 20개 가까이 되어(손잡이 1 + 붓
  종류 8 + 즐겨찾기 8 + 되돌리기/다시실행 2 + 최소화 1) verticalScroll이 필요할 만큼 길고, 그러면
  Surface 자신이 컨테이너 높이 전체로 늘어나면서 맨 위 항목인 손잡이가 항상 "화면 위쪽 가장자리
  근처"에도 동시에 있게 된다 — 즉 LEFT 도킹의 손잡이는 왼쪽 가장자리와 위쪽 가장자리 둘 다에 이미
  바짝 붙어 있는 셈이라, "지금 위치가 어느 가장자리에 더 가까운가"라는 기준 자체가 아무리 마진을
  손봐도 손잡이의 우연한 시작 위치에 계속 휘둘렸다. `nearestDock`을 컨테이너 크기·위치와 완전히
  무관한 "순 드래그 이동량(dx, dy) 중 더 큰 축, 그 방향"만 보는 방식으로 다시 썼다 — 최소 이동
  거리(32dp)를 못 넘으면 제자리, 넘으면 그 방향으로 무조건 도킹. `DragHandle`의
  onGloballyPositioned/positionInRoot 배선과 각 화면의 toolbarDragOrigin/containerRootPos를 전부
  걷어내 코드도 단순해졌다. `ToolbarDockTest.kt`를 새 방식에 맞게 다시 써서 LEFT/RIGHT↔TOP/BOTTOM
  왕복을 직접 검증하는 테스트를 추가했다. versionCode 128, 로컬 빌드 후 `gh release create`로 배포.
- **v2.9.23 배포** (2026-08-29, Claude): "세로모드(LEFT/RIGHT 도킹)에서 가로모드(TOP/BOTTOM)로
  전환이 안 된다"는 후속 리포트를 수정 — v2.9.22의 절대 위치 추적 자체는 맞았지만, `nearestDock`이
  다른 방향이 지금 도킹보다 2%p 더 가까워야만 옮기도록 고정 마진을 두고 있었다. LEFT/RIGHT 도킹의
  손잡이는 버튼바 안 맨 위 항목이라 자기 도킹된 가장자리에서 불과 수십 px밖에 안 떨어져 있어서
  현재 도킹의 거리 비율 자체가 이미 매우 작은데, 거기서 다시 2%p를 더 빼야 하니 TOP/BOTTOM으로
  건너가려면 반대쪽 모서리 몇 px 안까지 손잡이를 몰아넣어야 하는 수준으로 문턱이 비현실적으로
  높았다. 마진을 없애고 순수 strict "<" 비교만 남겼다 — 동률(정중앙 등) 방지는 그걸로 충분함을
  기존 테스트 4개로 손 계산해 확인했고, 손잡이가 자기 가장자리에 바짝 붙은 채 대각선으로 반대축까지
  드래그하는 새 회귀 테스트(`edgeHuggingHandleStillReachesThePerpendicularAxisOnANaturalDiagonalDrag`)
  를 추가했다. versionCode 127, 로컬 빌드 후 `gh release create`로 배포.
- **v2.9.22 배포** (2026-08-29, Claude): 버튼바 좌/우, 상/하 도킹이 비대칭으로 안 붙던 진짜 근본
  원인을 찾아 수정 — `nearestDock`이 "컨테이너 중앙 + 누적 드래그 델타"로 현재 위치를 재구성했는데,
  실제 드래그 손잡이의 화면 시작 위치는 도킹 방향마다 다르다(TOP/BOTTOM 도킹일 땐 손잡이가 화면
  왼쪽 가장자리 근처, LEFT/RIGHT 도킹일 땐 세로 중앙 근처). 그래서 손잡이가 이미 가까운 방향으로는
  화면 끝까지 금방 닿아버리고 반대 방향은 화면 전체를 가로질러야 해서, 실제로 도달 가능한 드래그
  거리가 비대칭이었음. 이게 "가로모드에서 좌측 고정 안 됨(우측은 됨), 세로모드에서 상단 고정 안
  됨(하단은 됨)"이라는 정확한 재현 리포트를 그대로 설명함. `DragHandle`이 드래그 시작 시
  `onGloballyPositioned`+`positionInRoot()`로 손가락의 실제 루트 좌표를 보고하도록 바꾸고,
  `nearestDock`은 델타 대신 이 절대 위치를 받도록 시그니처 변경. 스케치북/일기/공유 3개 화면과
  Interactive Preview(`BrushToolbarPreview.kt`)에 동일하게 배선, `ToolbarDockTest.kt`에
  "같은 절대 위치면 시작 도킹이 달라도 같은 결과"를 직접 검증하는 회귀 테스트 추가. versionCode 126,
  로컬 빌드 후 `gh release create`로 배포.
- **v2.9.21 배포** (2026-08-29, Claude): 버튼바 좌/우 도킹이 안 되던 진짜 원인을 찾아 수정 —
  `nearestDock`이 화면 중앙 기준 픽셀 거리로 네 방향을 비교하는데, 가로로 넓은 태블릿에서는 세로
  (위/아래) 쪽 절반 길이가 훨씬 짧아서 좌우로 크게 드래그해도 위/아래 거리가 계속 더 작게 이겨
  좌우 도킹에 사실상 도달할 수 없었다(기기 재현으로 확인). 각 축을 "절반 길이" 대비 비율로
  정규화해서 화면 비율과 무관하게 네 방향이 공평하게 겨루도록 수정하고, 미세한 드래그로 매번 LEFT로
  튀지 않도록 2%p 히스테리시스 추가. `ToolbarDockTest.kt`로 회귀 방지 테스트 추가. versionCode 125,
  로컬 빌드 후 `gh release create`로 배포.
- **v2.9.20 배포** (2026-08-29, Claude): 버튼바 TOP 도킹 상단 여백 10dp→4dp. 굵기/불투명도 그립의
  터치 영역을 시각적 pill(6dp)보다 넓게(28dp) 분리해서 잡기 쉽게 함 — 그 부작용으로 그립이 도킹
  반대쪽으로 밀려 보이던 회귀도 도킹별 앵커 계산으로 같이 수정. RIGHT + 펼침 상태에서도 TOP처럼
  ScreenControls(우측 상단 고정)와 안 겹치도록 정렬을 TopEnd로 바꾸고 위쪽 여백 확보(개인/공유/일기
  3개 화면). 그리기 화면들에 시스템 제스처 제외 영역(`excludeSystemGestureEdges`) 적용 — 버튼바를
  좌/우 가장자리로 끌 때 시스템 뒤로가기 스와이프에 터치를 뺏기던 문제 완화(읽기모드에 있던 처리를
  공용 Modifier로 뽑아 재사용). "좌우 도킹이 반대로 붙는다"는 재현 리포트 검증용으로 버튼바
  드래그-도킹 로직만 독립 실행할 수 있는 Interactive Preview 추가(`BrushToolbarPreview.kt`, 실시간
  디버그 텍스트 포함) — 실제 그리기 화면엔 그동안 `@Preview`가 전혀 없었음. versionCode 124, 로컬
  빌드 후 `gh release create`로 배포.
- **v2.9.19 배포** (2026-08-29, Claude): 홈 읽기모드가 가로모드 전용인데 PageCurl은 landscape에서
  항상 TwoPageSpread(펼친 책처럼 좌우 두 쪽)로 그린다는 걸 확인 — 상자 비율을 `book.size.ratio`
  하나에서 `*2`로 고쳐서 실제 기기에서도 생기던 이중 레터박스를 줄였다. 프리뷰 전용 정적 표시도
  좌/우 두 쪽 스프레드로 다시 그리고 페이지 번호 라벨 + 이전/다음 펼침면 버튼을 추가해 Interactive
  Preview로 넘겨볼 수 있게 함. 레터박스 여백 색은 종이 텍스처 색에 맞추려던 시도(고정값 → 텍스처
  평균색, `paperAverageColor`)를 여러 차례 시도했지만 질감 없는 단색은 실제 텍스처 옆에서 아무리
  맞춰도 이음새가 보였다 — 결국 홈 탭이 원래 쓰는 배경색(`MaterialTheme.colorScheme.background`)을
  그대로 써서 종이처럼 흉내내지 않고 자연스러운 UI 배경으로 정리(`paperAverageColor` 삭제). 프리뷰에
  가로모드(태블릿 비율) 5종 추가. versionCode 123, 로컬 빌드 후 `gh release create`로 배포.
- **v2.9.18 배포** (2026-08-29, Claude): 홈 읽기모드 레터박스를 없애고 다시 좌우 꽉 차게(레터박스
  여백에 베이지색 홈 배경이 비치던 게 진짜 문제였음). 목록/공유 탭 가로모드 표지: 길게누름=수정,
  더블탭=스케치 모드 진입으로 서로 맞바꿈(기존과 반대). 공유그리기 선생님모드 버튼 아이콘을 "지금
  상태"가 아니라 "누르면 할 동작" 기준으로 교체(공유 중엔 unshared 아이콘 = 끄기 동작을 보여줌).
  목록/공유 탭 3열 페이지 썸네일을 더블탭하면 그 페이지를 펼친 채로 스케치 모드가 열리도록
  `RootViewModel.openBookAtPage`/`SketchbookCanvasScreen`·`SharedBookScreen`의 `startPage` 배선
  추가. versionCode 122, 로컬 빌드 후 `gh release create`로 배포.
- **v2.9.17 배포** (2026-08-29, Claude): 홈 화면 가로모드를 2열 읽기모드(PageCurl로 표지 그 자리에서
  페이지 넘겨 읽기, 책 비율에 맞게 라운드 코너 처리)+3열 표지 목록(큰 표지+제목/날짜, 탭하면 2열
  대상 변경)으로 재구성, 세로모드는 기존 캐러셀 유지. 공유그리기 "선생님모드"가 게스트 화면에 반영
  안 되던 버그 수정 — host와 다른 페이지를 보고 있으면 오버레이 조건이 계속 거짓이던 게 원인이라,
  선생님모드가 켜진 동안 게스트가 host 페이지를 자동으로 따라가게 함. `EdgeGripSlider`/`DragHandle`의
  stale-closure 버그 수정(rememberUpdatedState) — 그립 슬라이더 원점 튐, 버튼바 좌우 도킹이 잘 안
  붙던 문제의 실제 원인이었음. 목록/공유 탭 가로모드 표지 더블탭으로 수정화면 바로 진입. 지우개 블러
  패널 우측에 전체지우기 버튼 이동(기존 툴바 버튼 제거). 회전/읽기모드/레이어/라쏘/전체화면/
  선생님모드 아이콘 6종을 사용자가 제공한 SVG(Lucide 아이콘셋) 그대로 커스텀 `ImageVector`로 포팅
  (`LineIcons.kt`). 구형 일기(투명 저장 기능 이전에 만들어진 일기)를 다시 열어 편집하면 종이 질감이
  `content`에 눌러붙어서 "투명 배경으로 저장"해도 배경이 같이 나오던 버그 수정 — 옛 그림은 paper
  자리에 고정 배경으로 깔고 content는 새로 투명하게 시작하도록 변경. 그리는 중 선이 버튼바를 지나가면
  숨기는 실험 기능은 그립 레인과 겹쳐서 되돌림. versionCode 121, 로컬 빌드 후 `gh release create`로
  배포.
...
## Next (2026-09-14)
- 검토 문서의 P1 순서로 내용 변경/저장 계약, 공통 클릭 접근성, 빈 화면 생성 안내와 대비를 개선한다.
- 기존 R.jar 점유/권한 문제를 해결한 뒤 테스트·lint를 재실행하고, 실제 기기에서 Undo/Redo 재열기·
  두 기기 공유·다크 모드·작은 화면·큰 글자 크기를 확인한다.
- 수정 전 기존 미커밋 `SketchbookScreens.kt` 표지 삭제 버튼 배치와 PPTX 작업을 보존한다.
- 아래 Phase 2~4 목록은 초기 로드맵 기록이다. 생성·다이어리·공유·계정 등 이미 구현된 내용을
  미구현으로 간주하지 말고 최신 코드와 검토 문서를 기준으로 다음 작업을 정한다.

### 초기 Phase 2~4 로드맵 (역사 기록)
...
## Decisions
- **2026-09-14 검토 범위**: 이번 작업은 현재 코드 구조 문서화와 UI·UX 평가다. 기존 종이·문구
  브랜드와 가로 미리보기·아이콘 중심·롱프레스 도킹 결정은 유지하고, 변경 후보는 근거와 함께
  검토 문서에 남긴다. 앱 동작 변경이나 전면 리팩터링은 이번 문서 정리에 포함하지 않았다.
- 색상 팔레트는 7색 라이브러리 최대 30개 중 최대 3개 선택(선택 순서=팔레트 순서, 4번째 선택 시
  가장 먼저 고른 것을 선입선출로 자동 해제) 구조로 확정. 라이브러리 색상 개수(7)와 활성 개수(3)는
  이번 스펙에서 고정값 — 사용자가 바꿀 수 있는 옵션으로 만들지 않는다. 기존 21색 팔레트 데이터는
  마이그레이션하지 않고 버린다.
...
- 레거시 다이어리의 검정 배경 보정은 저장 파일에 절대 쓰지 않고 표시용 복사본에서만 수행한다.
  가장자리 연결·최소 면적·넓은 행 조건을 모두 만족할 때 종이를 보이게 하되 원본 비검정 픽셀을
  먼저 유지하고 companion 필기는 그 위에 추가한다. 검정 배경과 그 안의 검정 선은 합성본만으로
  구분할 수 없으므로, 원본/companion/원격 사본 어느 곳에도 없는 정보는 복원됐다고 주장하지 않는다.
- 버튼바에는 별도 손잡이를 두지 않고 Surface 전체의 롱프레스를 이동 제스처로 사용한다. 짧은 탭은
  기존 버튼에 전달하고, 롱프레스 성립 뒤에만 Surface가 포인터를 소비한다. 도킹 대상은 드래그 중
  전체 누적 이동량의 지배 축·방향으로 계속 갱신해, 롱프레스 직후 작은 흔들림이 최종 방향을 잠그지
  않게 한다. 12dp 미만 이동은 제자리 유지한다.
...
## Open / Blockers
- **2026-09-14 검토에서 확인한 미수정 문제**: BrushView Undo/Redo 내용 변경이 저장/공유
  콜백으로 전달되지 않는다. 공통 bounceClick에는 접근성 클릭 semantics가 없고 주요 도구가
  30dp다. 빈 홈/목록 안내가 서로 반대이며 다크 로고·기본 표지 제목 대비가 낮다. 자세한 위치와
  재현 검증 절차는 `docs/reviews/2026-09-14-code-ui-ux-review.md`를 참고한다.
- **2026-09-14 실행 검증 제한**: 연결된 Android 기기 없음. 단위 테스트·lint 실행은 R.jar 삭제
  실패로 중단됨(점유 프로세스 미특정). 로컬 설정은 외부 PageCurl 모듈을 사용하므로 깨끗한
  체크아웃 검증과 구분해야 한다.
- **색상 라이브러리 디바운스 푸시 취소 엣지케이스** (2026-09-08): `ColorLibrarySync.kt`의 색상/이름
  편집 푸시는 500ms 디바운스인데, 그 안에 화면을 벗어나면(백그라운드/네비게이션) 대기 중이던 잡이
  취소돼 푸시가 안 나가고, 다음 reconcile pass가 오래된 원격 값으로 로컬 편집을 덮어쓸 수 있다.
  영향 범위는 라이브러리 1개의 마지막 편집으로 한정되고 발생 확률이 낮아 최종 리뷰에서 fix 없이
  park함 — 필요해지면 `DiaryScreens.kt`류의 flush-on-background 훅 추가를 고려.
...
- **2026-09-01 에뮬레이터 UI 검증 환경**: Galaxy Z Fold8 Main/Cover API 34 AVD가 ADB 연결 후
  Android 콜드 부팅 도중 호스트에서 반복 종료됨(swiftshader 및 GPU off 모두 동일). 다이어리 복구
  로직은 회귀 테스트·전체 테스트·APK·lint로 검증했지만, 합성 손상 PNG를 달력에 주입해 복구본을
  다시 pull하는 종단 픽셀 검증은 정상 동작하는 에뮬레이터 또는 실기기에서 한 번 더 확인할 것.
