...
- **22일 이전 그림일기 검은 배경 자동 복구** (2026-09-01, Codex): 과거 다이어리 합성본의 투명
  영역이 백업 과정에서 JPEG로 변환되며 검정으로 굳은 것이 원인이었다(스케치북 페이지에는 이미
  적용됐던 alpha 보존 수정이 다이어리 합성본에는 적용되지 않았던 레거시 데이터 문제). 달력
  썸네일 또는 상세 화면에서 파일을 처음 읽을 때, 가장자리와 연결되고 전체의 15% 이상이며 한 행의
  70% 이상을 덮는 큰 근검정 영역만 손상으로 판정한다. 별도 `_content.png`가 남아 있으면 수채화
  종이와 다시 합성해 검은 선까지 정확히 보존하고, 없는 구버전 파일은 판정된 검정 배경 픽셀만
  제거한 뒤 종이를 복원한다. 복구본은 불투명 PNG로 즉시 재저장되어 이후 썸네일/상세에서 정상
  표시되고, 갱신된 mtime 덕분에 다음 계정 동기화에서 손상된 원격본도 교체된다. 정상 내부 검정
  그림과 얇은 검정 테두리를 손상으로 오인하지 않는 회귀 테스트 3개 추가. 전체 단위 테스트 113개
  통과, `assembleDebug` 성공, `lintDebug` 성공(오류 0; 기존 경고만 존재). 호스트 Android
  에뮬레이터가 콜드 부팅 중 반복 종료되어 실제 UI 픽셀 검증은 아래 Blocker에 기록.
...
## Decisions
- 레거시 다이어리 복구는 날짜(예: 22일 이전)를 하드코딩하지 않고 이미지 내용으로 판정한다.
  정상 검정 선 보호를 위해 가장자리 연결·최소 면적·넓은 행 조건을 모두 만족해야 하며, 투명 필기
  companion이 있으면 픽셀 추정보다 companion 재합성을 우선한다.
