...
## Done
- **버튼바 LEFT/TOP 도킹 방향 잠금 수정** (2026-08-29, Codex): TOP/BOTTOM 도킹의 손잡이는
  화면 왼쪽 가까이, LEFT/RIGHT 도킹의 손잡이는 화면 위쪽 가까이에서 시작하기 때문에 LEFT/TOP으로
  끌 때 해당 축 이동이 화면 경계에서 먼저 멈추고, 손을 떼기 전까지 누적된 직교축 흔들림이 최종
  이동량을 이겨 TOP→LEFT가 BOTTOM으로 가거나 세로→TOP이 실패했다. 손을 뗄 때 최종 델타를 다시
  판정하지 않고, 12dp 임계값을 처음 넘은 순간의 방향을 제스처 종료까지 잠그도록 `DragHandle`에
  공통 적용했다. 개인 스케치북·공유 스케치북·그림일기와 두 Interactive Preview가 모두 확정된
  도킹 방향을 받도록 콜백을 통일했다. LEFT/TOP 초기 의도가 이후 BOTTOM/좌우 이동으로 덮어써지지
  않는 회귀 테스트 2개를 추가했고, 잠금 한 줄을 제거하면 두 테스트가 실제로 실패하는 mutation
  확인까지 마쳤다. 에뮬레이터에 새 debug APK 설치 성공; 사적 화면 캡처 반출은 보안 정책상 중단했다.
...
## Decisions
- 버튼바 도킹 대상은 손을 뗄 때의 최종 누적 이동량이 아니라, 12dp 임계값을 처음 넘은 순간의
  지배적인 축·방향으로 확정한다. 화면 가장자리에 가까운 손잡이에서 한 축이 일찍 막혀도 이후 손가락
  흔들림이 최초 의도를 바꾸지 않게 하기 위함이다.
