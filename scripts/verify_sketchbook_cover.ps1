...
$homeSource = Get-Content -Raw -Encoding UTF8 -LiteralPath $homePath
$listSource = Get-Content -Raw -Encoding UTF8 -LiteralPath $listPath
...
Assert-Contains $homeSource 'SketchbookCover(' 'Home must use SketchbookCover.'
Assert-Contains $listSource 'SketchbookCover(' 'Sketchbook list must use SketchbookCover.'
if ($theme.Contains('CoverColors')) { throw 'Theme.kt still contains CoverColors.' }
if ($homeSource.Contains('R.drawable.mascot_duck') -or $listSource.Contains('R.drawable.mascot_duck')) {
