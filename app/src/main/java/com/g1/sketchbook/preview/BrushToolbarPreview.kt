...
// 같은 배선(barModifier, onDragBar/onDragBarEnd, nearestDock, RIGHT+펼침 시 TopEnd 정렬)을 그대로
// 옮겨와서, BrushView(실제 캔버스)나 SketchbookRepository 없이도 손잡이 드래그 → 도킹 로직만
// 독립적으로 켜볼 수 있게 했다. "오른쪽으로 끌면 왼쪽에 붙는다"는 재현 리포트를 눈으로 확인하려고
// 드래그 중 실시간으로 toolbarDragPx·거리 계산·최종 도킹 결과를 화면에 그대로 찍어준다.
        ...
        BoxWithConstraints(
            ...
            BrushControls(
                ...
                onDragBarEnd = {
                    ...
                    val after = nearestDock(before, toolbarDragPx, cwPx, chPx)
                    lastResult = "container=${cwPx.roundToInt()}x${chPx.roundToInt()}  drag=(${toolbarDragPx.x.roundToInt()},${toolbarDragPx.y.roundToInt()})  $before → $after"
                    toolbarDock = after
            ...
            // 디버그 전용 — 드래그 결과(원래 도킹/새 도킹, 컨테이너 크기, 누적 드래그량)를 그대로
            // 찍어서 "오른쪽으로 끌었는데 왼쪽에 붙는다"는 증상을 숫자로 직접 확인할 수 있게 함.
            Text(
                "dock=$toolbarDock  live drag=(${toolbarDragPx.x.roundToInt()},${toolbarDragPx.y.roundToInt()})\n$lastResult",
